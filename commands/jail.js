...
module.exports.execute = async (client, message, args) => {
  ...
  if (getRandomInt(100) == 99) {
    if (parseInt(args[0])) {
      return await message.channel.send(`_${args[0]} uses a "get out of jail free" card_`);
    } else {
      var name = args.join(" ");
      if (name == "") {
        return await message.channel.send(`_<@${message.author.id}> uses a "get out of jail free" card_`);
      }
      ...
      if (name != "@everyone") {
        return await message.channel.send(`_${name} uses a "get out of jail free" card_`);
      } else {
        return await message.channel.send("_Jailbreak! Everyone escapes from jail!_");
      }
    }
  } else {
    if (parseInt(args[0])) {
      ...
      var name = args.join(" ");
      if (name == "") {
        return await message.channel.send(`_Puts <@${message.author.id}> in jail._`);
      }
      //Replace with mention if possible
      message.channel.members.forEach(member => {
        if (member.displayName.toLowerCase().indexOf(name.toLowerCase()) != -1 || member.user.username.toLowerCase().indexOf(name.toLowerCase()) != -1) name = "<@" + member.id + ">";
